...
class JplRuleTest {
    ...
    static final PrologKb KB = kbFamily();

    @BeforeEach
    void refreshKb() {
        for (Map.Entry<String, Set<Predicate>> entry: KB.globalFunctor2FactSetMap.entrySet()) {
    ...
    void testFamilyRule1() {
        ...
        /* parent(?, ?) :- */
        final JplRule rule = new JplRule(FUNCTOR_PARENT, cache, KB);
    ...
    void testFamilyRule3() {
        ...
        /* grandParent(?, ?) :- */
        final JplRule rule = new JplRule(FUNCTOR_GRANDPARENT, cache, KB);
    ...
    void testFamilyRule8() {
        ...
        /* father(?, ?):- */
        final JplRule rule = new JplRule(FUNCTOR_FATHER, cache, KB);
    ...
    void testFamilyRule9() {
        /* #1: father(?, ?):- */
        final JplRule rule1 = new JplRule(FUNCTOR_FATHER, new HashSet<>(), KB);
        ...
        /* #1: father(?, ?):- */
        final JplRule rule2 = new JplRule(FUNCTOR_FATHER, new HashSet<>(), KB);
    ...
    void testCounterExample1() {
        /* father(?, ?):- */
        final JplRule rule = new JplRule(FUNCTOR_FATHER, new HashSet<>(), KB);
        assertTrue(rule.toString().contains("father(?,?):-"));
        assertTrue(rule.toCompleteRuleString().contains("father(X0,X1):-"));
        assertEquals(
                new Eval(null, 5, 16 * 16, 0),
        ...
        expected_counter_examples.removeAll(KB.getGlobalFactsByFunctor(FUNCTOR_FATHER));
        assertEquals(expected_counter_examples, update_result.counterExamples);
    }
    ...
    void testFamilyWithCopy1() {
        ...
        final JplRule rule1 = new JplRule(rule);
        ...
        final JplRule rule2 = new JplRule(rule);
        ...
        final JplRule rule3 = new JplRule(rule);
    ...
    void testFamilyWithCopy2() {
        ...
        /* #1: parent(?, ?) :- */
        final JplRule rule1 = new JplRule(FUNCTOR_PARENT, cache, KB);
        ...
        /* #2: parent(f2, d2) :- */
        final JplRule rule2 = new JplRule(rule1);
        ...
        final JplRule rule3 = new JplRule(rule1);
        assertEquals(Rule.UpdateStatus.NORMAL, rule3.boundFreeVars2NewVar(FUNCTOR_FATHER, ARITY_FATHER, 1, 0, 1));
